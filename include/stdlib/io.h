...
ObjectResult *io_print_to_function(VM *vm, int arg_count, const Value *args);

/* println_to(channel: string, value)  -> Result<nil>
 * Same as print_to but appends '\n'. */
ObjectResult *io_println_to_function(VM *vm, int arg_count, const Value *args);

/* ── Input — stdin ─────────────────────────────────────────────────────────── */

/* scan()  -> Result<string>
 * Reads exactly one character from stdin and discards the rest of the line. */
ObjectResult *io_scan_function(VM *vm, int arg_count, const Value *args);

/* scanln()  -> Result<string>
 * Reads from stdin up to (and discarding) the next '\n'.
 * Returns the line without the newline character. */
ObjectResult *io_scanln_function(VM *vm, int arg_count, const Value *args);
...
ObjectResult *io_nscan_function(VM *vm, int arg_count, const Value *args);
...
ObjectResult *io_scan_from_function(VM *vm, int arg_count, const Value *args);

/* scanln_from(channel: string)  -> Result<string>
 * Reads from the named channel up to (and discarding) the next '\n'. */
ObjectResult *io_scanln_from_function(VM *vm, int arg_count,
                                      const Value *args);

/* nscan_from(channel: string, n: int)  -> Result<string>
 * Reads up to <n> characters from the named channel,
 * stopping early on '\n'. */
ObjectResult *io_nscan_from_function(VM *vm, int arg_count, const Value *args);
