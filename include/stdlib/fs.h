 ...
 * or is a directory. */
ObjectResult *fs_remove_function(VM *vm, int arg_count, const Value *args);

/* rename(from: string, to: string)  -> Result<nil>
 * Moves / renames a file or directory. */
ObjectResult *fs_rename_function(VM *vm, int arg_count, const Value *args);

/* copy_file(from: string, to: string)  -> Result<nil>
 * Copies the contents of <from> to <to>, creating or truncating <to>. */
ObjectResult *fs_copy_file_function(VM *vm, int arg_count, const Value *args);

/* mkdir(path: string)  -> Result<nil>
 * Creates a single directory.  Fails if the parent does not exist. */
ObjectResult *fs_mkdir_function(VM *vm, int arg_count, const Value *args);
...
ObjectResult *fs_read_file_function(VM *vm, int arg_count, const Value *args);

/* write_file(path: string, content: string)  -> Result<nil>
 * Creates or truncates the file and writes <content> to it. */
ObjectResult *fs_write_file_function(VM *vm, int arg_count, const Value *args);

/* append_file(path: string, content: string)  -> Result<nil>
 * Opens the file in append mode and writes <content> to it. */
ObjectResult *fs_append_file_function(VM *vm, int arg_count, const Value *args);
