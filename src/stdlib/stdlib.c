...
{
	ObjectTypeRecord *t_nil = REC(NIL_TYPE);
	ObjectTypeRecord *t_bool = REC(BOOL_TYPE);
	ObjectTypeRecord *t_int = REC(INT_TYPE);
	ObjectTypeRecord *t_flt = REC(FLOAT_TYPE);
	ObjectTypeRecord *t_str = REC(STRING_TYPE);
	ObjectTypeRecord *t_any = REC(ANY_TYPE);
	ObjectTypeRecord *t_num = REC(NUMERIC_TYPE);
	ObjectTypeRecord *t_hash = REC(HASHABLE_TYPE);
	ObjectTypeRecord *t_tbl = REC(TABLE_TYPE);
	ObjectTypeRecord *t_fn = REC(FUNCTION_TYPE);
	ObjectTypeRecord *t_err = REC(ERROR_TYPE);
	ObjectTypeRecord *t_res = REC(RESULT_TYPE);
	ObjectTypeRecord *t_rnd = REC(RANDOM_TYPE);
	ObjectTypeRecord *t_file = REC(FILE_TYPE);
	ObjectTypeRecord *t_vec = REC(VECTOR_TYPE);
	ObjectTypeRecord *t_mat = REC(MATRIX_TYPE);
	ObjectTypeRecord *t_cmpl = REC(COMPLEX_TYPE);
	ObjectTypeRecord *t_rang = REC(RANGE_TYPE);
	ObjectTypeRecord *t_tup = REC(TUPLE_TYPE);
	ObjectTypeRecord *t_buf = REC(BUFFER_TYPE);
	ObjectTypeRecord *t_set = REC(SET_TYPE);

	// Compound types used across multiple groups.
	ObjectTypeRecord *res_nil = RES(t_nil);
	ObjectTypeRecord *res_any = RES(t_any);
	ObjectTypeRecord *res_int = RES(t_int);
	ObjectTypeRecord *res_str = RES(t_str);
	ObjectTypeRecord *res_flt = RES(t_flt);
	ObjectTypeRecord *arr_str = ARR(t_str);
	ObjectTypeRecord *arr_any = ARR(t_any);
	ObjectTypeRecord *arr_int = ARR(t_int);
	ObjectTypeRecord *arr_flt = ARR(t_flt);
	ObjectTypeRecord *arr_num = ARR(t_num);

	ObjectTypeRecord *res_arr_any = RES(arr_any);
	...
	// ---------------------------------------------------------------
	{
		Callable fns[] = {
			...
			{"format", format_function, 2, ARGS(t_str, TBL(t_str, t_any)), t_str},
		};
	...
	// ---------------------------------------------------------------
	{
		Callable methods[] = {
			...
			{"starts_with", string_starts_with_method, 2, ARGS(t_str, t_str), t_bool},
			{"ends_with", string_ends_with_method, 2, ARGS(t_str, t_str), t_bool},
			{"contains", string_contains_method, 2, ARGS(t_str, t_str), t_bool},
			{"replace", string_replace_method, 3, ARGS(t_str, t_str, t_str), t_str},
			{"split", string_split_method, 2, ARGS(t_str, t_str), arr_str},
			{"substring", string_substring_method, 3, ARGS(t_str, t_int, t_int), res_str},
			{"concat", string_concat_method, 2, ARGS(t_str, t_str), t_str},
	...
	// ---------------------------------------------------------------
	{
		Callable methods[] = {
			...
			{"concat", array_concat_method, 2, ARGS(arr_any, arr_any), res_arr_any},
			{"slice", array_slice_method, 3, ARGS(arr_any, t_int, t_int), res_arr_any},
			{"reverse", array_reverse_method, 1, ARGS(arr_any), res_arr_any},
			{"index", array_index_of_method, 2, ARGS(arr_any, t_any), res_int},
			{"map", array_map_method, 2, ARGS(arr_any, t_fn), res_arr_any},
			{"filter", array_filter_method, 2, ARGS(arr_any, t_fn), res_arr_any},
			{"reduce", array_reduce_method, 3, ARGS(arr_any, t_fn, t_any), res_any},
	...
	// ---------------------------------------------------------------
	{
		Callable methods[] = {
			...
			{"remove", table_remove_method, 2, ARGS(t_tbl, t_hash), res_any},
			{"get", table_get_method, 2, ARGS(t_tbl, t_hash), res_any},
			{"has_key", table_has_key_method, 2, ARGS(t_tbl, t_hash), t_bool},
			{"get_or_else", table_get_or_else_method, 3, ARGS(t_tbl, t_hash, t_any), t_any},
		};
	...
	// ---------------------------------------------------------------
	{
		Callable methods[] = {
			...
			{"float", random_float_method, 3, ARGS(t_rnd, t_num, t_num), t_flt},
			{"bool", random_bool_method, 2, ARGS(t_rnd, t_num), t_bool},
	...
	{
		Callable methods[] = {
			{"dot", vector_dot_method, 2, ARGS(t_vec, t_vec), t_flt},
			{"add", vector_add_method, 2, ARGS(t_vec, t_vec), t_vec},
			{"subtract", vector_subtract_method, 2, ARGS(t_vec, t_vec), t_vec},
			{"multiply", vector_multiply_method, 2, ARGS(t_vec, t_num), t_vec},
			{"divide", vector_divide_method, 2, ARGS(t_vec, t_num), t_vec},
			{"magnitude", vector_magnitude_method, 1, ARGS(t_vec), t_flt},
			{"normalize", vector_normalize_method, 1, ARGS(t_vec), t_vec},
			{"distance", vector_distance_method, 2, ARGS(t_vec, t_vec), t_flt},
			{"angle_between", vector_angle_between_method, 2, ARGS(t_vec, t_vec), t_flt},
			{"cross", vector_cross_method, 2, ARGS(t_vec, t_vec), t_vec},
			{"lerp", vector_lerp_method, 3, ARGS(t_vec, t_vec, t_num), t_vec},
			{"reflect", vector_reflect_method, 2, ARGS(t_vec, t_vec), t_vec},
			{"equals", vector_equals_method, 2, ARGS(t_vec, t_vec), t_bool},
			...
			{"dimension", vector_dimension_method, 1, ARGS(t_vec), t_int},
		};
		init_type_method_table(vm, &vm->vector_type, methods, ARRAY_COUNT(methods));

		Callable fns[] = {
			{"Vec", new_vector_function, 2, ARGS(t_int, arr_num), t_vec},
		};
	...
	// ---------------------------------------------------------------
	{
		Callable methods[] = {
			...
			{"scale", scale_complex_number_method, 2, ARGS(t_cmpl, t_num), t_cmpl},
		...
		Callable fns[] = {
			{"Complex", new_complex_function, 2, ARGS(t_num, t_num), t_cmpl},
		};
	...
	{
		Callable methods[] = {
			{"get", matrix_get_method, 3, ARGS(t_mat, t_int, t_int), t_flt},
			{"set", matrix_set_method, 4, ARGS(t_mat, t_int, t_int, t_num), t_nil},
			{"add", matrix_add_method, 2, ARGS(t_mat, t_mat), t_mat},
			{"sub", matrix_subtract_method, 2, ARGS(t_mat, t_mat), t_mat},
			{"mul", matrix_multiply_method, 2, ARGS(t_mat, t_mat), t_mat},
			{"scale", matrix_scale_method, 2, ARGS(t_mat, t_num), t_mat},
			{"transpose", matrix_transpose_method, 1, ARGS(t_mat), t_mat},
			{"determinant", matrix_determinant_method, 1, ARGS(t_mat), t_flt},
			{"inverse", matrix_inverse_method, 1, ARGS(t_mat), RES(t_mat)},
			{"trace", matrix_trace_method, 1, ARGS(t_mat), t_flt},
			{"rank", matrix_rank_method, 1, ARGS(t_mat), t_int},
			{"row", matrix_row_method, 2, ARGS(t_mat, t_int), t_vec},
			{"col", matrix_col_method, 2, ARGS(t_mat, t_int), t_vec},
			{"equals", matrix_equals_method, 2, ARGS(t_mat, t_mat), t_bool},
			{"copy", matrix_copy_method, 1, ARGS(t_mat), t_mat},
			{"to_array", matrix_to_array_method, 1, ARGS(t_mat), arr_any},
			{"mul_vec", matrix_multiply_vector_method, 2, ARGS(t_mat, t_vec), t_vec},
		...
		Callable fns[] = {
			{"Matrix", new_matrix_function, 2, ARGS(t_int, t_int), t_mat},
			{"IMatrix", new_matrix_identity_function, 1, ARGS(t_int), t_mat},
			{"AMatrix", new_matrix_from_array_function, 3, ARGS(t_int, t_int, arr_num), t_mat},
		};
	...
	{
		Callable methods[] = {
			{"get", get_tuple_method, 2, ARGS(t_tup, t_int), res_any},
			{"slice", slice_tuple_method, 3, ARGS(t_tup, t_int, t_int), t_tup},
			{"index", index_tuple_method, 2, ARGS(t_tup, t_any), res_int},
			{"is_empty", is_empty_tuple_method, 1, ARGS(t_tup), t_bool},
			{"to_array", to_array_tuple_method, 1, ARGS(t_tup), arr_any},
			{"first", first_tuple_method, 1, ARGS(t_tup), res_any},
			{"last", last_tuple_method, 1, ARGS(t_tup), res_any},
			{"contains", contains_tuple_method, 2, ARGS(t_tup, t_any), t_bool},
			{"equals", equals_tuple_method, 2, ARGS(t_tup, t_tup), t_bool},
		};
	...
	{
		Callable methods[] = {
			{"add", add_set_method, 2, ARGS(t_set, t_hash), res_nil},
			{"remove", remove_set_method, 2, ARGS(t_set, t_hash), res_nil},
			{"discard", discard_set_method, 2, ARGS(t_set, t_hash), t_nil},
			{"union", union_set_method, 2, ARGS(t_set, t_set), t_set},
			{"intersection", intersection_set_method, 2, ARGS(t_set, t_set), t_set},
			{"difference", difference_set_method, 2, ARGS(t_set, t_set), t_set},
			{"sym_difference", sym_difference_set_method, 2, ARGS(t_set, t_set), t_set},
			{"is_subset", is_subset_set_method, 2, ARGS(t_set, t_set), t_bool},
			{"is_superset", is_superset_set_method, 2, ARGS(t_set, t_set), t_bool},
			{"is_disjoint", is_disjoint_set_method, 2, ARGS(t_set, t_set), t_bool},
			{"contains", contains_set_method, 2, ARGS(t_set, t_hash), t_bool},
			{"is_empty", is_empty_set_method, 1, ARGS(t_set), t_bool},
			{"to_array", to_array_set_method, 1, ARGS(t_set), arr_any},
	...
	{
		Callable fns[] = {
			{"sleep_s", sleep_seconds_function, 1, ARGS(t_num), t_nil},
			{"sleep_ms", sleep_milliseconds_function, 1, ARGS(t_num), t_nil},
