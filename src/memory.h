 ...
 * `bytesAllocated` counter, potentially triggers garbage collection if allocation
 * exceeds the `nextGC` threshold, and handles allocation failures.
 *
 * @param vm The virtual machine.
 * @param pointer The old block of memory to reallocate. If `NULL`, it's equivalent to `malloc`.
 * @param oldSize The old size of the memory block. If zero, it's equivalent to `malloc`.
 * @param newSize The new size of the memory block. If zero, it's equivalent to `free`.
 *
 * @return A pointer to the reallocated memory block. Returns `NULL` if `newSize` is zero.
 *         Exits the program if allocation fails and `realloc` returns `NULL` when `newSize` is not zero.
 */
 ...
 * freed by the garbage collector. If the object is not already marked, it is marked
 * and added to the gray stack for further processing in the mark phase.
 *
 * @param vm The virtual machine.
 * @param object The object to mark. If `NULL`, the function returns immediately.
 */
 ...
 *
 * If the given `value` is an object, this function calls `markObject` to mark it.
 * Primitive values are ignored as they are not managed by the garbage collector.
 *
 * @param vm The virtual machine.
 ...
 * are still allocated. It iterates through the object list and frees each object
 * using `freeObject`. It also frees the gray stack.
 *
 * @param vm The virtual machine.
 */
