			      ...
			      const Value *args)
{
	...
	if (!arrayAdd(vm, array, toAdd, array->size)) {
		return newErrorResult(
			vm,
			newError(vm,
				 copyString(vm, "Failed to add to array.", 23),
				 RUNTIME, false));
	}
				...
				const Value *args)
{
	...
	if (insertAt > array->size) {
		return newErrorResult(
			vm, newError(vm,
				     copyString(vm, "<index> is out of bounds.",
						25),
				     BOUNDS, false));
	}
			       ...
			       const Value *args)
{
	...
	if (startIndex > array->size) {
		return newErrorResult(
			vm,
			newError(vm,
				 copyString(vm, "<start_index> out of bounds.",
					    28),
				 BOUNDS, false));
	}
	...
	if (endIndex > array->size) {
		return newErrorResult(
			vm,
			newError(vm,
				 copyString(vm, "<end_index> out of bounds.",
					    26),
				 BOUNDS, false));
	}
	...
	if (endIndex < startIndex) {
		return newErrorResult(
			vm,
			newError(vm,
				 copyString(vm, "indexes out of bounds.", 22),
				 BOUNDS, false));
	}
			     ...
			     const Value *args)
{
	...
	for (uint32_t i = 0; i < array->size; i++) {
		...
		InterpretResult res;
		ObjectResult *result = executeUserFunction(vm, closure, 1,
							   &res);
				...
				const Value *args)
{
	...
	for (uint32_t i = 0; i < array->size; i++) {
		...
		ObjectResult *result = executeUserFunction(vm, closure, 2,
							   &res);
