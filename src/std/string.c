...
ObjectResult *stringFirstMethod(VM *vm, int argCount, Value *args) {
  ...
  if (string->length == 0) {
    return newErrorResult(
        vm, newError(vm,
                     copyString(vm,
                                "'string' must have at least one character to "
  ...
  if (string->length == 0) {
    return newErrorResult(
        ...
        newError(
            ...
            copyString(
                vm,
                "'string' must have at least one character to get a character.",
                61),
            VALUE, false));
  }

  return newOkResult(vm, OBJECT_VAL(copyString(
...
ObjectResult *stringUpperMethod(VM *vm, int argCount, Value *args) {
  ...
  if (buffer == NULL) {
    return newErrorResult(
        vm, newError(vm, copyString(vm, "Memory allocation failed.", 26),
                     MEMORY, false));
  }
...
ObjectResult *stringSubstringMethod(VM *vm, int argCount, Value *args) {
  ...
  if (!IS_INT(args[1])) {
    return newErrorResult(
        vm,
        newError(vm, copyString(vm, "<start> index must be of type 'int'.", 36),
                 VALUE, false));
  }
  if (!IS_INT(args[2])) {
    return newErrorResult(
        vm,
        newError(vm, copyString(vm, "<end> index must be of type 'int'.", 34),
                 VALUE, false));
  }
...
ObjectResult *stringSplitMethod(VM *vm, int argCount, Value *args) {
  if (!IS_CRUX_STRING(args[1])) {
    return newErrorResult(
        vm, newError(
                vm, copyString(vm, "<delimiter> must be of type 'string'.", 37),
                TYPE, false));
  }
  ...
  if (delimiter->length == 0) {
    return newErrorResult(
        vm, newError(vm, copyString(vm, "<delimiter> cannot be empty.", 28),
                     TYPE, false));
  }
  ...
  if (prefixTable == NULL) {
    return newErrorResult(
        vm, newError(vm, copyString(vm, "Memory allocation failed.", 25),
                     MEMORY, false));
  }
  ...
  if (prefixTable == NULL) {
    return newErrorResult(
        vm, newError(
                vm,
                copyString(
                    vm, "Failed to allocate memory for string.contains().", 48),
                MEMORY, false));
  }
...
ObjectResult *stringReplaceMethod(VM *vm, int argCount, Value *args) {
  if (!IS_CRUX_STRING(args[0]) || !IS_CRUX_STRING(args[1]) ||
    ...
    return newErrorResult(
        vm, newError(vm, copyString(vm, "All arguments must be strings.", 30),
                     TYPE, false));
  }
  ...
  if (string->length == 0 || goal->length == 0) {
    return newErrorResult(
        ...
        newError(
            ...
            string->length == 0
                ? copyString(
                      vm, "Source string must have at least one character.", 47)
                : copyString(
                      vm,
...
ObjectResult *stringStartsWithMethod(VM *vm, int argCount, Value *args) {
  ...
  if (!IS_CRUX_STRING(args[1])) {
    return newErrorResult(
        ...
        newError(vm,
                 copyString(
                     vm, "First argument <char> must be of type 'string'.", 47),
                 TYPE, false));
  }
