...
static Value castArray(VM *vm, const Value *args, bool *success) {
  ...
  if (IS_CRUX_STRING(value)) {
    ...
    ObjectArray *array = newArray(vm, string->length, vm->currentModuleRecord);
  ...
  if (IS_CRUX_TABLE(value)) {
    ...
    ObjectArray *array = newArray(vm, table->size * 2, vm->currentModuleRecord);
    uint32_t index = 0;
...
static Value castTable(VM *vm, const Value *args) {
  ...
  if (IS_CRUX_ARRAY(value)) {
    ...
    ObjectTable *table = newTable(vm, (int)array->size, moduleRecord);
  ...
  if (IS_CRUX_STRING(value)) {
    const ObjectString *string = AS_CRUX_STRING(value);
    ObjectTable *table = newTable(vm, (int)string->length, moduleRecord);
    for (uint32_t i = 0; i < string->length; i++) {
