  ...
  for (int i = 0; i < vm->args.argc; i++) {
    ...
    if (arg == NULL) {
      ...
      return newErrorResult(
          vm,
          newError(
              vm, copyString(vm, "Failed to allocate memory for argument.", 39),
              MEMORY, false));
    }
...
ObjectResult *getEnvFunction(VM *vm, int argCount, const Value *args) {
  if (!IS_CRUX_STRING(args[0])) {
    return newErrorResult(
        vm,
        newError(
            vm, copyString(vm, "Argument <name> must be of type 'string'.", 41),
            TYPE, false));
  }
  const char *value = getenv(AS_C_STRING(args[0]));
  if (value == NULL) {
    return newErrorResult(
        vm, newError(vm, copyString(vm, "Environment variable not found.", 31),
                     RUNTIME, false));
  }
  ...
  if (newValue == NULL) {
    return newErrorResult(
        ...
        newError(
            vm,
            copyString(
                vm, "Failed to allocate memory for environment variable value.",
                57),
            MEMORY, false));
  }
...
ObjectResult *sleepFunction(VM *vm, int argCount, const Value *args) {
  if (!IS_INT(args[0])) {
    return newErrorResult(
        vm,
        newError(
            vm, copyString(vm, "Argument <seconds> must be of type 'int'.", 41),
            MEMORY, false));
  }
