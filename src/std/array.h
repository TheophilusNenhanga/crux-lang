...
ObjectResult* arrayFilterMethod(VM *vm, int argCount,
                                const Value *args); // [1,2,3].filter(fn (x){ return x % 2 == 0;}) -> [2]

ObjectResult* arrayReduceMethod(VM *vm, int argCount,
                                const Value *args); // [1,2,3].reduce(fn (acc, x){ return acc + x;}, 0) -> 6

ObjectResult* arraySortMethod(VM *vm, int argCount,
                      const Value *args); // [1,2,3].sort() -> [1,2,3]

ObjectResult* arrayJoinMethod(VM *vm, int argCount, const Value *args); // [1, 2, 3].join("") -> "123"
