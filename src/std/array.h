...
ObjectResult *
arrayPopMethod(VM *vm, int argCount,
	       const Value *args); // [1,2,3].pop([1,2,3]) -> 3, [1,2]
ObjectResult *
arrayInsertMethod(VM *vm, int argCount,
		  const Value *args); // [1,3].insert(1, 2) -> [1,2,3]
ObjectResult *
arrayRemoveAtMethod(VM *vm, int argCount,
...
ObjectResult *arrayMapMethod(
	VM *vm, int argCount,
	const Value *args); // [1,2,3].map(fn (x){ return x * 2;}) -> [2,4,6]

ObjectResult *arrayFilterMethod(
	VM *vm, int argCount,
	const Value
		*args); // [1,2,3].filter(fn (x){ return x % 2 == 0;}) -> [2]

ObjectResult *arrayReduceMethod(
	VM *vm, int argCount,
	const Value
...
ObjectResult *arrayJoinMethod(VM *vm, int argCount,
