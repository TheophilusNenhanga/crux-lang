...
ObjectResult *sqrtFunction(VM *vm, int argCount, Value *args) {
	...
	if (number < 0) {
		return newErrorResult(vm, newError(vm,
...
ObjectResult *lnFunction(VM *vm, int argCount, Value *args) {
	...
	double number = AS_FLOAT(args[0]);
	if (number < 0) {
		return newErrorResult(vm, newError(vm, copyString(vm, "Cannot calculate natural logarithm of non positive number.", 58),
		                              VALUE, false));
	}
...
ObjectResult *log10Function(VM *vm, int argCount, Value *args) {
	...
	double number = AS_FLOAT(args[0]);
	if (number < 0) {
		return newErrorResult(vm, newError(vm, copyString(vm, "Cannot calculate base 10 logarithm of non positive number.", 58),
		                              VALUE, false));
	}
