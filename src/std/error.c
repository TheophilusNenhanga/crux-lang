			      ...
			      const Value *args)
{
	...
	case BRANCH_EXTENT: {
		ObjectString *type = copyString(vm, "<branch extent error>",
						21);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	case CLOSURE_EXTENT: {
		ObjectString *type = copyString(vm, "<closure extent error>",
						22);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	...
	case ARGUMENT_EXTENT: {
		ObjectString *type = copyString(vm, "<argument extent error>",
						23);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	...
	case COLLECTION_EXTENT: {
		ObjectString *type = copyString(vm, "<collection extent error>",
						25);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	case VARIABLE_EXTENT: {
		ObjectString *type = copyString(vm, "<variable extent error>",
						23);
		return newOkResult(vm, OBJECT_VAL(type));
	}

	case RETURN_EXTENT: {
		ObjectString *type = copyString(vm, "<return extent error>",
						21);
		return newOkResult(vm, OBJECT_VAL(type));
	}

	case ARGUMENT_MISMATCH: {
		ObjectString *type = copyString(vm, "<argument mismatch error>",
						22);
		return newOkResult(vm, OBJECT_VAL(type));
	}

	case STACK_OVERFLOW: {
		ObjectString *type = copyString(vm, "<stack overflow error>",
						22);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	case COLLECTION_GET: {
		ObjectString *type = copyString(vm, "<collection get error>",
						22);
		return newOkResult(vm, OBJECT_VAL(type));
	}

	case COLLECTION_SET: {
		ObjectString *type = copyString(vm, "<collection set error>",
						22);
		return newOkResult(vm, OBJECT_VAL(type));
	}

	case UNPACK_MISMATCH: {
		ObjectString *type = copyString(vm, "<unpack mismatch error>",
						23);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	...
	case IMPORT_EXTENT: {
		ObjectString *type = copyString(vm, "<import extent error>",
						21);
		return newOkResult(vm, OBJECT_VAL(type));
	}
	case IO: {
