...
static Callable arrayMethodsArray[] = {{"pop", arrayPopMethod, 1},
...
static Callable tableMethodsArray[] = {{"values", tableValuesMethod, 1},
                                       {"keys", tableKeysMethod, 1},
                                       {"pairs", tablePairsMethod, 1},
                                       {NULL, NULL, 0}};

static Callable errorMethodsArray[] = {{"message", errorMessageMethod, 1},
                                       {"type", errorTypeMethod, 1},
                                       {NULL, NULL, 0}};

static Callable randomMethodsArray[] = {
    {"seed", randomSeedMethod, 2},     {"int", randomIntMethod, 3},
    {"double", randomDoubleMethod, 3}, {"bool", randomBoolMethod, 2},
    {"choice", randomChoiceMethod, 2}, {NULL, NULL, 0}};

static InfallibleCallable randomInfallibleMethodsArray[] = {
    {"_next", randomNextMethod, 1}, {NULL, NULL, 0}};

static Callable fileMethodsArray[] = {
    {"readln", readlnFileMethod, 1}, {"read_all", readAllFileMethod, 1},
    {"write", writeFileMethod, 2},   {"writeln", writelnFileMethod, 2},
    {"close", closeFileMethod, 1},   {NULL, NULL, 0}};

static Callable coreFunctionsArray[] = {{"scanln", scanlnFunction, 0},
                                        {"panic", panicFunction, 1},
                                        {"len", lengthFunction, 1},
                                        {"error", errorFunction, 1},
                                        {"assert", assertFunction, 2},
                                        {"err", errorFunction, 1},
                                        {"ok", okFunction, 1},
                                        {"int", intFunction, 1},
...
static Callable mathFunctionsArray[] = {
    ...
    {"round", roundFunction, 1}, {NULL, NULL, 0}};

static InfallibleCallable mathInfallibleFunctionsArray[] = {
    {"_e", eFunction, 0}, {"_pi", piFunction, 0}, {NULL, NULL, 0}};

static Callable ioFunctionsArray[] = {{"print_to", printToFunction, 2},
                                      {"scan", scanFunction, 0},
...
static InfallibleCallable randomInfallibleFunctionsArray[] = {
    {"Random", randomInitFunction, 0}, {NULL, NULL, 0}};

static Callable systemFunctionsArray[] = {{"args", argsFunction, 0},
...
bool initializeStdLib(VM *vm) {
  ...
  if (!registerNativeInfallibleFunctions(vm, &vm->currentModuleRecord->globals,
                                         coreInfallibleFunctionsArray)) {
    return false;
  }
