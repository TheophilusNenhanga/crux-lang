...
static bool unwrap_result_or_panic(VM *vm, const Value value, Value *out)
{
	ObjectModuleRecord *current_module_record = vm->current_module_record;

	if (!IS_CRUX_RESULT(value)) {
		runtime_panic(current_module_record, TYPE, "Expected Result from specialized binary operation.");
		return false;
	}
	...
	if (!result->is_ok) {
		...
		runtime_panic(current_module_record, error->type, "%s", error->message->chars);
		return false;
	}
...
static bool push_direct_binary_result(VM *vm, const Value value)
{
	ObjectModuleRecord *current_module_record = vm->current_module_record;
	pop_two(current_module_record);
...
static bool push_unwrapped_binary_result(VM *vm, const Value value)
{
	ObjectModuleRecord *current_module_record = vm->current_module_record;
	Value unwrapped;
	if (!unwrap_result_or_panic(vm, value, &unwrapped)) {
		return false;
	}
	pop_two(current_module_record);
	push(current_module_record, unwrapped);
	return true;
}

bool specialized_binary_operation(VM *vm, const OpCode operation)
{
	ObjectModuleRecord *current_module_record = vm->current_module_record;
	const Value right = PEEK(current_module_record, 0);
	const Value left = PEEK(current_module_record, 1);

	switch (operation) {
	case OP_ADD_INT:
		return int_add(current_module_record, AS_INT(left), AS_INT(right));
	case OP_ADD_NUM:
		return float_add(current_module_record, TO_DOUBLE(left), TO_DOUBLE(right));
	case OP_SUBTRACT_INT:
		return int_subtract(current_module_record, AS_INT(left), AS_INT(right));
	case OP_SUBTRACT_NUM:
		return float_subtract(current_module_record, TO_DOUBLE(left), TO_DOUBLE(right));
	case OP_MULTIPLY_INT:
		return int_multiply(current_module_record, AS_INT(left), AS_INT(right));
	case OP_MULTIPLY_NUM:
		return float_multiply(current_module_record, TO_DOUBLE(left), TO_DOUBLE(right));
	case OP_DIVIDE_NUM:
		return float_divide(current_module_record, TO_DOUBLE(left), TO_DOUBLE(right));
	case OP_INT_DIVIDE_INT:
		return int_int_divide(current_module_record, AS_INT(left), AS_INT(right));
	case OP_MODULUS_INT:
		return int_modulus(current_module_record, AS_INT(left), AS_INT(right));
	case OP_POWER_INT:
		return int_power(current_module_record, AS_INT(left), AS_INT(right));
	case OP_POWER_NUM:
		return float_power(current_module_record, TO_DOUBLE(left), TO_DOUBLE(right));
	case OP_ADD_VECTOR_VECTOR: {
		const Value args[] = {left, right};
		return push_unwrapped_binary_result(vm, vector_add_method(vm, args));
	}
	case OP_SUBTRACT_VECTOR_VECTOR: {
		const Value args[] = {left, right};
		return push_unwrapped_binary_result(vm, vector_subtract_method(vm, args));
	}
	...
	case OP_SUBTRACT_MATRIX_MATRIX: {
		return push_unwrapped_binary_result(vm, matrix_subtract_value(vm, AS_CRUX_MATRIX(left), AS_CRUX_MATRIX(right)));
	}
	case OP_ADD_MATRIX_SCALAR:
		return push_direct_binary_result(vm, matrix_scalar_add_value(vm, AS_CRUX_MATRIX(left), TO_DOUBLE(right)));
	case OP_ADD_SCALAR_MATRIX:
		return push_direct_binary_result(vm, matrix_scalar_add_value(vm, AS_CRUX_MATRIX(right), TO_DOUBLE(left)));
	case OP_SUBTRACT_MATRIX_SCALAR:
		return push_direct_binary_result(vm, matrix_scalar_subtract_value(vm, AS_CRUX_MATRIX(left), TO_DOUBLE(right)));
	case OP_SUBTRACT_SCALAR_MATRIX:
		return push_direct_binary_result(vm, scalar_matrix_subtract_value(vm, TO_DOUBLE(left), AS_CRUX_MATRIX(right)));
	case OP_MULTIPLY_MATRIX_MATRIX: {
		return push_unwrapped_binary_result(vm, matrix_multiply_value(vm, AS_CRUX_MATRIX(left), AS_CRUX_MATRIX(right)));
	}
	case OP_MULTIPLY_MATRIX_SCALAR:
		return push_direct_binary_result(vm, matrix_scale_value(vm, AS_CRUX_MATRIX(left), TO_DOUBLE(right)));
	case OP_MULTIPLY_SCALAR_MATRIX:
		return push_direct_binary_result(vm, matrix_scale_value(vm, AS_CRUX_MATRIX(right), TO_DOUBLE(left)));
	case OP_DIVIDE_MATRIX_SCALAR:
		return push_unwrapped_binary_result(vm, matrix_scalar_divide_value(vm, AS_CRUX_MATRIX(left), TO_DOUBLE(right)));
	default:
		runtime_panic(current_module_record, RUNTIME, "Unknown specialized binary operation %d.", operation);
		return false;
	}
}
